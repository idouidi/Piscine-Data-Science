...
class Model:
    ...
    def __init__(self, classifier: RandomForestClassifier, accuracy: float, precision: float, recall: float, f1: float) -> None:
        self.classifier: RandomForestClassifier = classifier
...
def display_tree(model: Model) -> None:
    ...
    try:
        ...
        plot_tree(
            model.classifier.estimators_[0],
...
def save_predictions(model: Model, dataset: DataSet) -> None:
    """Generate predictions and save to Tree.txt"""
    try:
        if model is None:
            raise ValueError("Model is None")
        ...
        print(f"IOError: Cannot write to Tree.txt - {e}")
    except Exception as e:
        print(f"Error saving predictions: {e}")


def main() -> None:
    """Predict the knight's alignment using a Random Forest Classifier and evaluate the model."""
    try:
        if len(sys.argv) != 3:
            raise ValueError("Usage: python3 Tree.py <Training_knight.csv> <Test_knight.csv>")
